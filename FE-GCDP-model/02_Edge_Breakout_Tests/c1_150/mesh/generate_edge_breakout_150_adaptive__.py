...
csv_tolerance = 10.0                     # Distance radius to match current elements to the imported centroids

# Far Field Parameters
far_x = 250.0        # Size of far field in -x direction
far_z = 150.0        # Size of far field in -z direction
ff_interval = 3

# Concrete Slab
edge_dist = 150.0     # Distance from anchor center to the free edge (+x direction)
nearfield_x = edge_dist + 75.0  # Length of the near field region in the x-direction (from free edge to start of far field)
...
# Mesh Parameters
mesh_size_steel = 4.0
mesh_size_concrete_outer = 13.0  # Base size for the concrete block
...
if refine_sleeve:
    ...
    borehole_hexes = [h for h in borehole_hexes if cubit.get_center_point("hex", h)[1] > (-anchor_d - 100.01)]
